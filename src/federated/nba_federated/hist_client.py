...
over teams as mean × K. DP noise N(0, noise_sd²) is added before sending.

State kept between rounds in context.state: training and validation margins,
the number of trees applied, and each shot's current node.
...
from nba_federated.task import get_global_make_rate, partition_frames

STATE_KEY = "hist"
_DATA: dict[tuple, tuple] = {}          # per-actor cache: binned local train / validation data


def _local_data(pid: int, n_parts: int, strategy: str, seed: int, bin_stride: int):
    key = (pid, n_parts, strategy, seed, bin_stride)
    if key not in _DATA:
        X_tr, X_va, y_tr, y_va = partition_frames(pid, n_parts, strategy, random_state=seed)
...
class HistClient(Client):
    def __init__(self, context: Context):
        ...
        self.spec, self.b_tr, self.y_tr, self.b_va, self.y_va = _local_data(
            self.pid, int(context.node_config["num-partitions"]), str(rc.get("strategy", "team")),
            self.seed, int(rc.get("hist-bin-stride", 1)))
        m0 = float(np.log(get_global_make_rate() / (1 - get_global_make_rate())))
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        if instr.get("begin"):
            c.begin_tree(float(instr["subsample"]))
        else:
            c.gradients()
        if instr.get("advance") is not None:
            c.advance(*(np.asarray(a) for a in instr["advance"]))
        sd = float(instr["noise_sd"])
        if instr["phase"] == "leaf":
            G, H = c.leaf_sums(HG.Tree.from_dict(instr["structure"]), sd)
        else:
            G, H = c.histograms(int(instr["n_nodes"]), np.asarray(instr["feats"]), sd)
        self.node = c.node
        self._save()
        vec = np.concatenate([G.ravel(), H.ravel()])
        return FitRes(status=Status(code=Code.OK, message="OK"),
                      parameters=ndarrays_to_parameters([vec]), num_examples=1, metrics={})
