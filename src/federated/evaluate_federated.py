...
def main():
    ...
    c0 = summary.iloc[0]
    lines.append(f"Centralized (same params) & {c0['central_brier_mean']:.4f} & {c0['central_logloss_mean']:.4f} "
                 f"& {c0['central_auc_mean']:.4f} & -- \\\\")
    for _, s in summary.iterrows():
    ...
    if local_mean is not None:
        plot_convergence(curves, summary, float(summary["central_brier_mean"].mean()),
